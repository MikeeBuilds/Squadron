...
class Hippocampus:
    """
    The Long-Term Vector Memory (Hippocampus).
    Uses ChromaDB to store and recall memories semantically.
    """
    ...
    def __init__(self, persistence_path: str = None):
        ...
        Args:
            persistence_path: Path to store the vector DB. 
                              If None, attempts to find money-bot-discord/.gemini/chroma_db
        """
        if not persistence_path:
            # Smart default finding
            cwd = Path(os.getcwd())
            if (cwd / "money-bot-discord").exists():
                 base = cwd / "money-bot-discord" / ".gemini"
            else:
                 base = cwd / ".gemini"
                 
            self.db_path = base / "chroma_db"
        else:
            self.db_path = Path(persistence_path)
            
        # Ensure dir exists
        self.db_path.mkdir(parents=True, exist_ok=True)
            
        logger.info(f"🧠 Hippocampus initializing at {self.db_path}")
        
        self.client = chromadb.PersistentClient(path=str(self.db_path))

    def add_memory(self, collection_name: str, text: str, metadata: dict, memory_id: str):
        """
        Store a memory (embedding) in the specified collection.
        """
        try:
    ...
    def recall(self, collection_name: str, query_text: str, n_results: int = 3):
        """
        Recall memories semantically related to the query.
        """
        try:
            collection = self.client.get_or_create_collection(name=collection_name)
            results = collection.query(
    ...
    def list_memories(self, collection_name: str, limit: int = 10):
        """
        List recent memories (raw).
        """
        try:
            collection = self.client.get_or_create_collection(name=collection_name)
            ...
            return {}
>>>>>>> 4b417d9 (feat: upgrade brain to gemini-3-pro and add read_file tool)
